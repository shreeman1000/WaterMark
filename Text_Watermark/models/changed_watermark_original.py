...
def _binary_encoding_function(token):
    hash_value = int(hashlib.sha256(token.encode('utf-8')).hexdigest(), 16)
    random_bit = hash_value % 2
    return random_bit
...
class watermark_model:
    def __init__(self, language, mode, tau_word, lamda):
        self.device = torch.device('cuda:'+str(get_free_gpu()) if torch.cuda.is_available() else 'cpu')
    ...
    def watermark_embed(self,text):
        ...
        for masked_token_index in range(start_index+1, end_index-1):
            # pdb.set_trace()
            rad = str(random.randint(10000, 1000001))
            binary_encoding = 1 - binary_encoding_function(tokens[masked_token_index - 1] + rad + tokens[masked_token_index])
            print(tokens[masked_token_index - 1], " ", tokens[masked_token_index], " -> ", binary_encoding, " ", masked_token_index)
            ...
            for i, tok in enumerate(enhanced_candidates):
                binary_encoding = binary_encoding_function(tokens[masked_token_index - 1] + rad + tok[0])
