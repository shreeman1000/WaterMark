...
from RobustWatermark.train_watermark_model import TransformModel
import random
import os
import json
import jieba
...
class WatermarkContext(WatermarkBase):
    def __init__(
        ...
        mapping_file = "./RobustWatermark/data/mappings/300_mapping_32000.json"
    ...
    def detect(
        ...
        for i in range(1, len(word_2d)):
            ...
            if len(list(jieba.cut(context_sentence))) < 40:
                print(context_sentence, list(jieba.cut(context_sentence)))
                # continue
...
class WatermarkWindow(WatermarkBase):
    ...
    def detect(self, text: str = None):
        ...
        for i in range(self.window_size, len(input_ids)):
            ...
            total += 1
        def _compute_z_score(observed_count, T):
        ...
        return _compute_z_score(count, total)
